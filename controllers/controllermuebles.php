...
class ControllersMueble
{
    ...
    {
        include("backend/bd.php");
        $prestamo = ($bd->query("SELECT *, DATE_ADD(fecha_prestamo, INTERVAL duracion DAY) FROM prestamo_bien T LEFT JOIN bienes_publicos B ON 
    ...
    public function movimientosBienes($router)
    {
        ...
        if (empty($router->getParam())) { //Revisa que haya algun parametro
            ...
            if ($query->num_rows > 0) { //Si hay al menos un resultado el permiso esta dado a este cargo en referencia a esta acción    
                ...
                if (!$analista) {
                    $prestamo = ($bd->query("SELECT *, DATE_ADD(fecha_prestamo, INTERVAL duracion DAY) FROM prestamo_bien T LEFT JOIN usuario U ON T.solicitante=U.id 
                                            LEFT JOIN perfil P ON U.id=P.id_usuario LEFT JOIN departamento D 
                    ...
                    $prestamo = ($bd->query("SELECT *, DATE_ADD(fecha_prestamo, INTERVAL duracion DAY) FROM prestamo_bien T 
                                            INNER JOIN bienes_publicos B ON T.id_bien=B.id_bien 
                                            LEFT JOIN usuario U ON T.solicitante=U.id 
                                            LEFT JOIN perfil P ON U.id=P.id_usuario 
                                            LEFT JOIN departamento D ON P.departamento_id=D.departamento_id
                                            WHERE id_prestamo_bien = " . $router->getParam()))->fetch_assoc();

                    $responsable = ($bd->query("SELECT * FROM bienes_publicos B 
                                                LEFT JOIN usuario U ON B.responsable=U.id 
                                                LEFT JOIN perfil P ON U.id=P.id_usuario 
                                                LEFT JOIN departamento D ON P.departamento_id=D.departamento_id 
                                                WHERE id_bien = " . $prestamo["id_bien"]))->fetch_assoc();
